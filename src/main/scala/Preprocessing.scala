...
object Preprocessing {
  def tokenize(sentence : String, stopwordFlag : Boolean, stemFlag : Boolean) : List[String] = {
    var result = sentence.toLowerCase
      .replaceAll("(\\d)\\|([\\.\\/])\\|(\\d)", "$1$2$3")
      .replaceAll("([^\\s])\\|([\\-])\\|([^\\s])", "$1$2$3")
      .replaceAll("([A-Za-z])\\2+", "$1")
      .replaceAll("[-/]", " ")
      .replaceAll("'s" , "")
      .split(" ")
      .toList
    ...
    if(stopwordFlag) {
      ...
      result = result.filterNot(stopwords.toSet)
    }

    if(stemFlag) {
      result = result.map{ case word => PorterStemmer.stem(word) }
    }
    result
  }

  def stem(token : List[String]) : List[String] = {
    var result = List[String]()
    for(tok <- token) {
      result :+= PorterStemmer.stem(tok)
    }
    result
  }

  def removeStopwords(token : List[String]) : List[String] = {
    val stopwords = List[String]()
    token.filter(stopwords.contains(_))
  }

  def datasetSeparator(dataset : (List[String], List[Int]), percentTraining : Double) : (List[String],List[Int],List[String],List[Int]) = {
    val samplesize = (dataset._1.size/2).toInt
    val trainingsize = (percentTraining * dataset._1.size/2).toInt
    println("Training : Testing => " + percentTraining + " : "+ "%01.1f".format(1-percentTraining))
    println("Training size : " + trainingsize*2 + " , Testing size : " + (samplesize-trainingsize)*2 )
    val datatrain : List[String] = dataset._1.slice(0,trainingsize) ::: dataset._1.slice(samplesize, samplesize+trainingsize)
    val labeltrain : List[Int] = dataset._2.slice(0,trainingsize) ::: dataset._2.slice(samplesize, samplesize+trainingsize)
    val datatest : List[String] = dataset._1.slice(trainingsize, samplesize) ::: dataset._1.slice(samplesize+trainingsize, samplesize*2)
    val labeltest : List[Int] = dataset._2.slice(trainingsize, samplesize) ::: dataset._2.slice(samplesize+trainingsize, samplesize*2)
    (datatrain,labeltrain,datatest,labeltest)
  }

  def loadStopwords(): List[String] = {
    var stopwords = List[String]()
    val stopwordpath = "./dictionary/english-stopwords"
    for (line <- Source.fromFile(stopwordpath,"UTF-8").getLines) {
      stopwords :+= line
    }
    stopwords
  }

  def generateToken(data : List[String], stopwordFlag : Boolean, stemmerFlag : Boolean) : List[List[String]] = {
    var token = List[List[String]]()
    for(sentence <- data) {
      token :+= tokenize(sentence, stopwordFlag, stemmerFlag)
    }
    token
  }
